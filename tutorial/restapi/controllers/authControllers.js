...
module.exports.registerUser = catchAsyncErrors (async (req, res, next) => {
    ...
    const insertdata = [user.name, user.email, user.role, hashpassword, date];
    const insertcol = ['name', 'email', 'role', 'password', 'createdAt'];
...
module.exports.loginUser = catchAsyncErrors(async (req, res, next ) => {
    ...
    const query = 'select password, user_id from user where email = "' + email + '"';
        ...
        catchAsyncErrors(async function (err, rows) {
            if (err){
                ...
                if(rows.length < 1){
                    ...
                    //Create JWT Token
                    // const token = jwt.sign({ id: raw_user_id}, process.env.JWT_SECRET, {
                    //     expiresIn : process.env.JWT_EXPIRES_TIME
                    // });
