...
function EditPost(){
    ...
    useEffect(() => {
        if(state.sendCount){
            ...
            async function fetchPost() {
                try{
                    //call specific put api
                    //update post detail by id (await)
                    //sample Axios call with cancel midway
                    //const response = await Axios.get(`/post/${id}`, {cancelToken: ourRequest.token})
                    const response = {
                        data: 
    ...
    return (
        <Page title="Edit New Post">
            ...
            <form className="mt-3" onSubmit={submitHandler}>
                ...
                <button className="btn btn-primary" disable={state.isSaving}>Save Updates</button>
            </form>
        </Page>
    )
}
